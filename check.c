...
void DrawMenu() {
    // Draw the menu interface using raygui components
    BeginDrawing();
    ClearBackground(RAYWHITE);
    
    // Example menu buttons
    if (GuiButton((Rectangle){ SCREEN_WIDTH/2 - 50, SCREEN_HEIGHT/2 - 30, 100, 30 }, "Start Game")) {
        currentGameState = GAME;
    }
    if (GuiButton((Rectangle){ SCREEN_WIDTH/2 - 50, SCREEN_HEIGHT/2 + 10, 100, 30 }, "Exit")) {
...
void UpdateGame(Player *player, Bullet bullets[], Bullet enemyBullets[], Bullet h2bomb[], Enemy *enemy, Astroid *comet, Power *power, UnwantedAsteroid asteroids[], Barrier *barrier, Level *level, Music backgroundMusic) {
    // Update game logic, handle user input for game actions
    frameTime += GetFrameTime();
...
BeginDrawing();
    ...
    // Example game drawing
    DrawText("Game is Running!", SCREEN_WIDTH/2 - MeasureText("Game is Running!", 20)/2, SCREEN_HEIGHT/2 - 10, 20, DARKGRAY);
}
    ...
    while (!WindowShouldClose()) {
        ...
        switch (currentGameState) {
            case MENU:
                ...
                break;
            case GAME:
                UpdateGame(&player, bullets, enemyBullets, h2bomb, &enemy, &comet, &power, asteroids, &barrier, &level, backgroundMusic);
                DrawGame(&player, bullets, enemyBullets, h2bomb, &enemy, &comet, &power, asteroids, &barrier, &level, frames);
                break;
            case EXIT:
                CloseWindow();
                return 0;
